...
async function modalsalesreport(ref, room=''){
    ...
    if(rm){
        ...
        did('modaldetails').innerHTML = `
                ...
                <div class="col-span-2">
                    <p class="!text-sm font-semibold flex w-full justify-between">Description: <span class="uppercase !text-sm font-normal text-left w-[50%]">${data1.saleentry.description}</span></p>
                    ${data1.saleentry.ownerid < 0 ? '' : `<p class="!text-sm font-semibold flex w-full justify-between">Room / CC: <span class="uppercase !text-sm hidden font-normal text-left" style="">${data1.saleentry.ownerid}</span></p>`}
                    <p class="!text-sm font-semibold flex w-full justify-between">Total Amount: <span class="uppercase !text-sm font-normal text-left hidden" style="">${formatNumber(data1.saleentry.servicecharge)}</span></p>
                    <p class="!text-sm font-semibold flex w-full justify-between">Amount Paid: <span class="uppercase !text-sm font-normal text-left hidden" style="">${formatNumber(data1.amountreceived)}</span></p>
                    <p class="!text-sm font-semibold flex w-full justify-between">Ref: <span class="uppercase !text-sm font-normal text-left hidden" style="">${data1.saleentry.reference}</span></p>
                    <p class="!text-sm font-semibold flex w-full justify-between">Payment Method: <span class="uppercase !text-sm font-normal hidden text-left" style="">${data1.saleentry.paymentmethod}</span></p>
                    <p class="!text-sm font-semibold flex w-full justify-between">Transaction Date: <span class="uppercase !text-sm font-normal text-left" style="">${specialformatDateTime(data1.saleentry.transactiondate)}</span></p>
                </div>
            `;
            let tt = 0
