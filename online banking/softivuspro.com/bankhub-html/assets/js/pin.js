...
async function fetchmanagepin(id) {
    ...
    let request = await httpRequest2(`inventory/requisition/view?${queryParams ? `${queryParams}` : ''}${id ? `&id=${id}` : ''}`, null, document.querySelector('#viewrequisitionform #submit'), 'json', 'GET');
...
async function validatePassword() {
    ...
    let request = await httpRequest2(`auth/verifypasswordaccess`, param, document.querySelector('#confirmButton'), 'json', 'POST');
    if (request.status) {
        pinstate = "CURRENT"
        ...
        if(pinstate == "NEW"){
           getAndVerifyOTP().then(otpstatus => {
               ...
               if (otpstatus) {
                   ...
                   params.append('id', the_user.id);
                   httpRequest2(`admin/managepin`, params, null, 'json', 'POST').then(request => {
                       Swal.close();
