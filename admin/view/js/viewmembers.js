async function viewmembersActive() {
    ...
    await getAllUsers('createdby', 'staff', true, true)
    ...
    new TomSelect('#registrationpointsearch', {
        ...
        onInitialize: function() {
            ...
            if(!checkpermission('FILTER ALL REGISTRATION POINTS')) this.setTextboxValue('readonly', true);
        }
    });
...
async function fetchviewmembers() {
    ...
    let request = await httpRequest2(`api/v1/members/finduser?createdby=${did('createdby').tomselect.getValue()}&role=MEMBER&status=ACTIVE&registrationpoint=${did('registrationpointsearch').tomselect.getValue()??0}`, null,  document.querySelector('#viewmembersform #submit'), 'json', 'GET');
