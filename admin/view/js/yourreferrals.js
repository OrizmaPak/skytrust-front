async function yourreferralsActive() {
    ...
    //         if(!checkpermission('FILTER yourreferrals')) this.setValue(the_user.yourreferrals);
            // if(!checkpermission('FILTER yourreferrals')) this.setTextboxValue('readonly', true);
    //     }
    // });
...
async function onyourreferralsTableDataSignal() {
    ...
    <tr>
        ...
        <td class="flex items-center gap-3 ">
            <button title="Edit row entry" onclick="fetchyourreferralsview('${item.id}')" class="material-symbols-outlined rounded-full bg-green-400 h-8 w-8 text-white drop-shadow-md text-xs" style="font-size: 18px;">visibility</button>
            <button title="Delete row entry" onclick="removeyourreferrals('${item.id}')" class="hidden material-symbols-outlined rounded-full bg-red-600 h-8 w-8 text-white drop-shadow-md text-xs" style="font-size: 18px;">delete</button>
        </td>
    </tr>`
    )
    .join('')
